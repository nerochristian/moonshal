...
class WhitelistStore:
    ...
    @staticmethod
    def _normalize_duration_seconds(duration_seconds: Optional[int]) -> Optional[int]:
    ...
    @staticmethod
    def _calculate_expiry_iso(duration_seconds: Optional[int], *, now: Optional[datetime] = None) -> Optional[str]:
        normalized_seconds = WhitelistStore._normalize_duration_seconds(duration_seconds)
        if normalized_seconds is None:
            return None
        base_time = now or datetime.now(UTC)
        return (base_time + timedelta(seconds=normalized_seconds)).isoformat()
    ...
    async def ensure_initialized(self) -> None:
        ...
        async with self._init_lock:
            ...
            self._initialized = True
        await self._expire_stale_users()
    ...
    async def create_key(
        ...
        *,
        ...
        normalized_duration_days = (
            None
            if normalized_duration_seconds is None
            else max(1, (normalized_duration_seconds + 86399) // 86400)
        )
        effective_duration_days = (
            normalized_duration_days if normalized_duration_days is not None else self.luarmor_key_days
        )
        if self.uses_luarmor_keys:
            ...
            created = await self.luarmor.create_user(
                ...
                key_days=effective_duration_days,
            )
            ...
            if self._pool is not None:
                await self._pool.execute(
                    """
                    INSERT INTO keys (key, created_by, created_at, used, duration_days, duration_seconds)
                    VALUES ($1, $2, $3, 0, $4, $5)
                ...
                def _run() -> None:
                    with self._sqlite_connect() as connection:
                        connection.execute(
                            """
                            INSERT OR IGNORE INTO keys (key, created_by, created_at, used, duration_days, duration_seconds)
                            VALUES (?, ?, ?, 0, ?, ?)
                            """,
                            (
                                user_key,
    ...
    async def create_keys(
        ...
        *,
        ...
        return [
            await self.create_key(created_by, duration_seconds=duration_seconds)
    ...
    async def redeem_key(self, discord_id: int | str, key: str) -> bool:
        ...
        if self._pool is not None:
            async with self._pool.acquire() as connection:
                async with connection.transaction():
                    row = await connection.fetchrow(
                        "SELECT used, duration_days, duration_seconds FROM keys WHERE key = $1 FOR UPDATE",
                        normalized_key,
                    )
                    if row is None or int(row["used"]) != 0:
                        return False
                    access_expires_at = self._calculate_expiry_iso(
                        (
                            ...
                            if row["duration_seconds"] is not None
                                ...
                                else int(row["duration_days"]) * 86400
                            )
                        ),
                        now=current_time,
                    )
                    await connection.execute(
                        """
                        INSERT INTO users (discord_id, key, joined_at, access_expires_at)
                        VALUES ($1, $2, $3, $4)
            ...
            try:
                ...
                await self._rollback_redeem(discord_id, normalized_key, previous_user)
        ...
        def _run() -> bool:
            with self._sqlite_connect() as connection:
                row = connection.execute(
                    "SELECT used, duration_days, duration_seconds FROM keys WHERE key = ?",
                    (normalized_key,),
                ).fetchone()
                if row is None or int(row["used"]) != 0:
                    return False
                ...
                connection.execute(
                    ...
                    SET key = excluded.key,
                        access_expires_at = excluded.access_expires_at
                    """,
                    (discord_id_str, normalized_key, now, access_expires_at),
                )
                connection.execute(
    ...
    async def whitelist_user(self, discord_id: int | str, *, created_by: int | str | None) -> str:
        if await self.is_blacklisted(discord_id):
            raise ValueError("Blacklisted users cannot be whitelisted.")
    ...
    async def delete_unused_key(self, key: str) -> bool:
        await self.ensure_initialized()
        normalized_key = key.strip()
        ...
        def _run() -> bool:
            with self._sqlite_connect() as connection:
                cursor = connection.execute(
                    "DELETE FROM keys WHERE key = ? AND used = 0",
                    (normalized_key,),
                )
                connection.commit()
    ...
    async def _init_postgres(self) -> None:
        ...
        async with self._pool.acquire() as connection:
            ...
            await connection.execute(
                ...
                ALTER TABLE keys ADD COLUMN IF NOT EXISTS duration_days INTEGER;
                ALTER TABLE keys ADD COLUMN IF NOT EXISTS duration_seconds INTEGER;
                """
            )
    ...
    def _init_sqlite(self) -> None:
        ...
        with self._sqlite_connect() as connection:
            ...
            self._ensure_sqlite_key_columns(connection)
            connection.commit()
    ...
    def _ensure_sqlite_key_columns(self, connection: sqlite3.Connection) -> None:
        existing_columns = {
            str(row["name"])
    ...
    async def _rollback_redeem(
        ...
        discord_id: int | str,
        new_key: str,
        previous_user: Optional[dict[str, Any]],
    ) -> None:
        await self.ensure_initialized()
        discord_id_str = str(discord_id)
        previous_key_str = (
            None if previous_user is None or previous_user.get("key") in (None, "")
            else str(previous_user.get("key"))
        )
        previous_access_expires_at = (
            None if previous_user is None or previous_user.get("access_expires_at") in (None, "")
            else str(previous_user.get("access_expires_at"))
        )
    ...
    async def _rollback_unwhitelist(
        ...
        access_expires_at = previous_user.get("access_expires_at")
