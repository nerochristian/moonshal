...
class WhitelistStore:
    ...
    @staticmethod
    def _normalize_duration_days(duration_days: Optional[int]) -> Optional[int]:
        if duration_days is None:
            return None
        normalized = int(duration_days)
        return normalized if normalized > 0 else None

    @staticmethod
    def _calculate_expiry_iso(duration_days: Optional[int], *, now: Optional[datetime] = None) -> Optional[str]:
        normalized_days = WhitelistStore._normalize_duration_days(duration_days)
        if normalized_days is None:
            return None
        base_time = now or datetime.now(UTC)
        return (base_time + timedelta(days=normalized_days)).isoformat()
    ...
    async def create_key(
        ...
        *,
        ...
        effective_duration = (
            normalized_duration if normalized_duration is not None else self.luarmor_key_days
        )
        if self.uses_luarmor_keys:
            ...
            if self._pool is not None:
                await self._pool.execute(
                    """
                    INSERT INTO keys (key, created_by, created_at, used, duration_days)
                    VALUES ($1, $2, $3, 0, $4)
                ...
                def _run() -> None:
                    with self._sqlite_connect() as connection:
                        connection.execute(
                            """
                            INSERT OR IGNORE INTO keys (key, created_by, created_at, used, duration_days)
                            VALUES (?, ?, ?, 0, ?)
                            """,
                            (user_key, created_by_str, timestamp, effective_duration),
                        )
                        connection.commit()
                    ...
                    def _run() -> None:
                        with self._sqlite_connect() as connection:
                            connection.execute(
                                ...
                                VALUES (?, ?, ?, 0, ?)
                                """,
                                (key, created_by_str, timestamp, normalized_duration),
                            )
                            connection.commit()
    ...
    async def create_keys(
        ...
        *,
        ...
        return [
            await self.create_key(created_by, duration_days=duration_days)
    ...
    async def redeem_key(self, discord_id: int | str, key: str) -> bool:
        ...
        if self._pool is not None:
            async with self._pool.acquire() as connection:
                async with connection.transaction():
                    row = await connection.fetchrow(
                        "SELECT used, duration_days FROM keys WHERE key = $1 FOR UPDATE",
                        normalized_key,
                    )
                    if row is None or int(row["used"]) != 0:
                        return False
                    access_expires_at = self._calculate_expiry_iso(
                        None if row["duration_days"] is None else int(row["duration_days"]),
                        now=current_time,
                    )
                    await connection.execute(
                        """
        ...
        def _run() -> bool:
            with self._sqlite_connect() as connection:
                row = connection.execute(
                    "SELECT used, duration_days FROM keys WHERE key = ?",
                    (normalized_key,),
                ).fetchone()
                if row is None or int(row["used"]) != 0:
                    return False
